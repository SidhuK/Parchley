...
public struct PageSelection: Codable, Hashable, Sendable {
    ...
    public init(pages: [Int] = []) throws {
        guard pages.allSatisfy({ $0 > 0 }) && pages == pages.sorted() && Set(pages).count == pages.count else {
            throw DomainError.invalidPageSelection
        }
        self.pages = pages
    }
    public static let all = try! PageSelection()
}
...
public struct JobRequest: Codable, Sendable {
    ...
    public init(documentID: DocumentID, attemptID: AttemptID, stagedInputPath: URL, outputDirectory: URL,
        ...
        guard stagedInputPath.isFileURL, outputDirectory.isFileURL else { throw DomainError.invalidPath }
...
public enum JobStage: String, Codable, Sendable { case readingPDF, recognizingScannedPages, writingResult }
...
public enum DomainError: Error, Sendable { case invalidPageSelection, invalidPath }
