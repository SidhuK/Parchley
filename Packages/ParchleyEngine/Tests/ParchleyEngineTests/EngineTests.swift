...
final class EngineTests: XCTestCase {
    func testAdapterPreservesAttemptID() throws {
        ...
        let request = try JobRequest(documentID: DocumentID(), attemptID: id,
    ...
    func testRustEngineConvertsNativeTextFixture() async throws {
        ...
        let request = try JobRequest(documentID: DocumentID(), attemptID: AttemptID(), stagedInputPath: input, outputDirectory: output, ocrMode: .off)
        let engine = try RustEngine(libraryURL: URL(fileURLWithPath: path))
        let handle = try engine.start(request)
        var terminal: JobSnapshot?
        for try await snapshot in SnapshotPoller(intervalNanoseconds: 50_000_000).snapshots(for: handle) { terminal = snapshot }
        XCTAssertEqual(terminal?.state, .completed, terminal?.failure?.message ?? "no failure detail")
        let result = try handle.resultDescriptor()
        XCTAssertTrue(try String(contentsOf: result.markdownURL, encoding: .utf8).contains("Parchley bridge"))
    }
}
...
private func makeFixturePDF() -> Data {
    ...
    var pdf = "%PDF-1.4\n".data(using: .utf8)!
    var offsets = [0]
    for object in objects { offsets.append(pdf.count); pdf.append(object.data(using: .utf8)!) }
    let xref = pdf.count
    var tail = "xref\n0 6\n0000000000 65535 f \n"
    for offset in offsets.dropFirst() { tail += String(format: "%010d 00000 n \n", offset) }
    tail += "trailer\n<< /Size 6 /Root 1 0 R >>\nstartxref\n\(xref)\n%%EOF\n"
    pdf.append(tail.data(using: .utf8)!)
    return pdf
}

private final class TestHandle: EngineJobHandleBackend {
    func snapshot() throws -> JobSnapshot { JobSnapshot(attemptID: AttemptID(), revision: 0, state: .queued) }
    func requestCancel() {}
