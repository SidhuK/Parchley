...
public struct JobHandle: Sendable {
    ...
    init(attemptID: AttemptID, backend: any EngineJobHandleBackend) { self.attemptID = attemptID; self.backend = backend }
...
private enum RustLibraryError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .libraryNotFound(let detail): return "The bundled Rust library could not be opened. \(detail)"
        case .missingSymbol(let name): return "The bundled Rust library is missing \(name)."
        case .engineCreationFailed: return "The bundled Rust engine could not be created."
        case .runtimeConfigurationFailed(let detail): return "The bundled OCR runtime could not be configured: \(detail)"
        }
    }
}
...
public struct RustEngine: ParchleyEngine, Sendable {
    ...
    public func releaseJob(attemptID: AttemptID) {
        // The Rust handle remains retained by Swift until its terminal snapshot
        // has been consumed. This call is reserved for future bookkeeping.
        _ = attemptID
    }
...
private final class RustLibrary: @unchecked Sendable {
    ...
    private let freeEngine: FreeEngine
    let startFunction: Start
    let snapshotFunction: Snapshot
    let cancelFunction: Cancel
    let resultFunction: Result
    let freeJob: FreeJob
    let freeString: FreeString
    private let shutdownFunction: Shutdown
    ...
    init(url: URL?) throws {
        ...
        self.freeEngine = try symbol("parchly_engine_free", FreeEngine.self)
        self.startFunction = try symbol("parchly_engine_start_json", Start.self)
        self.snapshotFunction = try symbol("parchly_job_snapshot_json", Snapshot.self)
        self.cancelFunction = try symbol("parchly_job_cancel", Cancel.self)
        self.resultFunction = try symbol("parchly_job_result_json", Result.self)
        self.freeJob = try symbol("parchly_job_free", FreeJob.self)
        self.freeString = try symbol("parchly_string_free", FreeString.self)
        self.shutdownFunction = try symbol("parchly_engine_shutdown", Shutdown.self)
        ...
        self.engine = engine
        let configureRuntime: ConfigureRuntime = try symbol("parchly_engine_configure_runtime", ConfigureRuntime.self)
        ...
        if FileManager.default.fileExists(atPath: pdfium.path), FileManager.default.fileExists(atPath: ort.path) {
            let error = pdfium.path.withCString { pdfiumPath in
                ort.path.withCString { ortPath in configureRuntime(engine, pdfiumPath, ortPath) }
            }
            if let error {
    ...
    deinit {
        // Job backends retain this object until their handles are released. The
        // Rust worker therefore cannot outlive the loaded code.
        freeEngine(engine)
        // Keep a dynamically loaded engine resident for the process lifetime.
        // Rust workers may still be unwinding after their Swift handle is gone,
        // and unloading code while they run would be unsafe. The small resident
        // library is preferable to a use-after-dlclose during cancellation.
    }

    func shutdown() { shutdownFunction(engine) }
}

private final class RustJobBackend: EngineJobHandleBackend, @unchecked Sendable {
    private let library: RustLibrary
    private let job: UnsafeMutableRawPointer
    ...
    init(library: RustLibrary, request: JobRequest) throws {
        ...
        guard let job = json.withCString({ library.startFunction(library.enginePointer, $0) }) else { throw EngineError.internalFailure }
        self.job = job
    }

    func snapshot() throws -> JobSnapshot {
        let wire: RustSnapshot = try decode(library.snapshotFunction(job))
        guard let attempt = UUID(uuidString: wire.attemptID) else { throw EngineError.internalFailure }
        let state: JobState
        switch wire.state {
        ...
        case "Writing result", "Complete": .writingResult
        default: nil
        }
        let failure = wire.error.map { JobFailure(code: Self.mapError($0.kind), message: $0.message) }
    ...
    func requestCancel() { library.cancelFunction(job) }
    func resultDescriptor() throws -> ResultDescriptor {
        let result: RustResult? = try decode(library.resultFunction(job))
        guard let result else { throw EngineError.outputUnavailable }
        guard let attempt = UUID(uuidString: result.attemptID) else { throw EngineError.internalFailure }
        return ResultDescriptor(attemptID: AttemptID(attempt), manifestURL: URL(fileURLWithPath: result.manifestPath), markdownURL: URL(fileURLWithPath: result.markdownPath))
    }
    deinit { library.freeJob(job) }
    ...
    private static func mapError(_ kind: String) -> EngineError {
        switch kind {
...
private extension RustLibrary {
    var enginePointer: UnsafeMutableRawPointer { engine }
}

private struct RustRequest: Encodable {
    let documentID: String; let attemptID: String; let inputPath: String; let outputDirectory: String
    let pageNumbers: [UInt32]?; let ocrMode: String; let modelDirectory: String?; let password: String?
    enum CodingKeys: String, CodingKey { case documentID = "document_id"; case attemptID = "attempt_id"; case inputPath = "input_path"; case outputDirectory = "output_directory"; case pageNumbers = "page_numbers"; case ocrMode = "ocr_mode"; case modelDirectory = "model_directory"; case password }
    init(_ request: JobRequest) {
        documentID = request.documentID.description; attemptID = request.attemptID.description
        inputPath = request.stagedInputPath.path; outputDirectory = request.outputDirectory.path
        pageNumbers = request.pageSelection.pages.isEmpty ? nil : request.pageSelection.pages.map(UInt32.init)
        ocrMode = request.ocrMode.rawValue.capitalized; modelDirectory = request.modelDirectory?.path; password = request.password
    }
}

private struct RustResult: Decodable {
    let attemptID: String; let manifestPath: String; let markdownPath: String
    enum CodingKeys: String, CodingKey { case attemptID = "attempt_id"; case manifestPath = "manifest_path"; case markdownPath = "markdown_path" }
}
private struct RustError: Decodable { let kind: String; let message: String }
private struct RustSnapshot: Decodable {
    let attemptID: String; let revision: UInt64; let state: String; let stage: String?
    let pagesCompleted: UInt32?; let pagesTotal: UInt32?; let error: RustError?
    enum CodingKeys: String, CodingKey { case attemptID = "attempt_id"; case revision; case state; case stage; case pagesCompleted = "pages_completed"; case pagesTotal = "pages_total"; case error }
}
